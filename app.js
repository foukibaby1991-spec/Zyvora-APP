...
window.saveAndRender=async function(label){await saveDay(label);renderApp();};
window.addWork=async function(label){await saveDay(label);const d=getDraft();let day=getDay(label);day.rows.push({type:"work",site:"",hours:"0",km:"0"});d.days=(d.days||[]).filter(x=>x.day!==label);d.days.push(day);setDraft(d);renderApp();};
window.addAbsence=async function(label){await saveDay(label);const d=getDraft();let day=getDay(label);day.rows.push({type:"absence",absence:"Maladie",hours:"0"});d.days=(d.days||[]).filter(x=>x.day!==label);d.days.push(day);setDraft(d);renderApp();};
window.addHoliday=async function(label){await saveDay(label);const d=getDraft();let day=getDay(label);day.rows=[{type:"holiday",name:"Jour férié"}];d.days=(d.days||[]).filter(x=>x.day!==label);d.days.push(day);setDraft(d);renderApp();};
window.removeRow=async function(label,index){await saveDay(label);const d=getDraft();let day=getDay(label);day.rows=day.rows.filter((_,i)=>i!==index);if(!day.rows.length)day.rows=[{type:"work",site:"",hours:"0",km:"0"}];d.days=(d.days||[]).filter(x=>x.day!==label);d.days.push(day);setDraft(d);renderApp();};
...
window.changeWeek=function(delta){weekOffset+=delta;renderApp();};
