...
function reportTotals(r){
  const days=r.days||[];
  return {
    h:days.reduce((s,d)=>s+num(d.hours),0),
    km:days.reduce((s,d)=>s+num(d.km),0),
    meal:days.reduce((s,d)=>s+num(d.meal),0),
    expense:days.reduce((s,d)=>s+num(d.expense),0)
  };
}
function prettyRows(day){
  const rows=day.rows&&day.rows.length?day.rows:[];
  if(rows.length){
    return rows.map(row=>{
      if(row.type==="holiday")return `<div class="line"><b>Jour férié</b><span>${esc(row.name||"Jour férié")} · 8.5 h</span></div>`;
      if(row.type==="absence")return `<div class="line"><b>${esc(row.absence||"Absence")}</b><span>${esc(row.hours||0)} h</span></div>`;
      return `<div class="line"><b>${esc(row.site||"Chantier")}</b><span>${esc(row.hours||0)} h · ${esc(row.km||0)} km</span></div>`;
    }).join("");
  }
  return `<div class="line"><b>${esc(day.note||"Détails")}</b><span>${esc(day.hours||0)} h · ${esc(day.km||0)} km</span></div>`;
}
window.downloadReportPdf=function(rstr){
  const r=JSON.parse(decodeURIComponent(rstr));
  const t=reportTotals(r);
  const totalFrais=t.meal+t.expense;
  const title=`Rapport d’heures - ${esc(r.employee_name||"")}`;
  const days=(r.days||[]).map(d=>`
    <section class="day">
      <div class="day-head"><h3>${esc(d.day||"")}</h3><strong>${num(d.hours).toFixed(2)} h</strong></div>
      ${prettyRows(d)}
      <div class="mini"><span>Repas: CHF ${num(d.meal).toFixed(2)}</span><span>Frais: CHF ${num(d.expense).toFixed(2)}</span>${d.ticket_path?`<span>Ticket: oui</span>`:`<span>Ticket: non</span>`}</div>
    </section>`).join("");
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>${title}</title><style>
    @page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#172033;margin:0;background:white}.header{background:linear-gradient(135deg,#111827,#243b66);color:white;padding:28px;border-radius:18px;margin-bottom:18px}.brand{font-size:30px;font-weight:900;letter-spacing:3px}.subtitle{opacity:.9;margin-top:6px}.info{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin:18px 0}.box{border:1px solid #e5e7eb;border-radius:14px;padding:14px;background:#f8fafc}.label{font-size:11px;text-transform:uppercase;color:#64748b;letter-spacing:.08em}.value{font-size:20px;font-weight:800;margin-top:4px}.status{display:inline-block;border-radius:999px;padding:6px 12px;background:#dcfce7;color:#166534;font-weight:700}.day{border:1px solid #e5e7eb;border-radius:16px;margin:10px 0;padding:14px;break-inside:avoid}.day-head{display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #edf2f7;padding-bottom:8px;margin-bottom:8px}h3{margin:0;font-size:18px}.line{display:flex;justify-content:space-between;gap:16px;padding:7px 0;border-bottom:1px dashed #e5e7eb}.line:last-child{border-bottom:0}.line span{color:#475569}.mini{display:flex;gap:10px;flex-wrap:wrap;margin-top:10px}.mini span{background:#eef2ff;border-radius:999px;padding:5px 9px;font-size:12px}.signatures{display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-top:32px}.sign{height:80px;border-top:1px solid #94a3b8;padding-top:8px;color:#64748b}.footer{margin-top:24px;text-align:center;color:#94a3b8;font-size:12px}@media print{.noprint{display:none}}
  </style></head><body>
    <div class="header"><div class="brand">ZYVORA</div><div class="subtitle">Rapport d’heures propre pour validation et archivage</div></div>
    <div class="info"><div class="box"><div class="label">Employé</div><div class="value">${esc(r.employee_name||"")}</div></div><div class="box"><div class="label">Période</div><div class="value">${fmtDate(r.week_start)} - ${fmtDate(r.week_end)}</div></div><div class="box"><div class="label">Statut</div><div class="value"><span class="status">${esc(r.status||"envoyé")}</span></div></div></div>
    <div class="info"><div class="box"><div class="label">Total heures</div><div class="value">${t.h.toFixed(2)} h</div></div><div class="box"><div class="label">Total kilomètres</div><div class="value">${t.km.toFixed(0)} km</div></div><div class="box"><div class="label">Repas + frais</div><div class="value">CHF ${totalFrais.toFixed(2)}</div></div></div>
    ${days}
    <div class="signatures"><div class="sign">Signature employé</div><div class="sign">Signature admin</div></div>
    <div class="footer">Document généré automatiquement par Zyvora · ${new Date().toLocaleDateString("fr-CH")}</div>
    <button class="noprint" onclick="window.print()" style="position:fixed;right:20px;bottom:20px;padding:12px 18px;border:0;border-radius:12px;background:#111827;color:white;font-weight:700;cursor:pointer">Imprimer / enregistrer en PDF</button>
    <script>setTimeout(()=>window.print(),500)<\/script>
  </body></html>`;
  const w=window.open("","_blank");
  if(!w)return alert("Autorise les pop-ups pour ouvrir le PDF.");
  w.document.open(); w.document.write(html); w.document.close();
};
function reportsTable(data,isAdmin){
  if(!data.length)return `<div class="empty">Aucun rapport.</div>`;
  return `<div class="table-wrap"><table class="table"><thead><tr><th>Période</th><th>Employé</th><th>Statut</th><th>Actions</th></tr></thead><tbody>${data.map(r=>`<tr><td>${fmtDate(r.week_start)} - ${fmtDate(r.week_end)}</td><td>${r.employee_name||""}</td><td>${badge(r.status)}</td><td class="actions"><button class="small secondary" onclick='alert(${JSON.stringify(JSON.stringify(r.days||[]))})'>Voir</button><button class="small secondary" onclick="downloadReportPdf('${encodeURIComponent(JSON.stringify(r))}')">PDF propre</button>${hasTickets(r)?`<button class="small secondary" onclick="showTickets('${encodeURIComponent(JSON.stringify(r))}')">Tickets</button>`:""}${isAdmin?`<button class="small ok" onclick="setReportStatus('${r.id}','validé')">Valider</button><button class="small danger" onclick="setReportStatus('${r.id}','refusé')">Refuser</button>`:""}</td></tr>`).join("")}</tbody></table></div>`;
}
